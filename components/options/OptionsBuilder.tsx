...
export function OptionsBuilder({ trade }: { trade?: TradeConfig }) {
  ...
  // Trade placement (only used when `trade` is provided).
  const [placing, setPlacing] = useState(false);
  const [placeMsg, setPlaceMsg] = useState<{ ok: boolean; msg: string } | null>(null);
  ...
  const strategyName = useMemo(
    () => (mode === "strategy" ? strategyById(strategyId)?.name ?? "Strategy" : recognizeStrategy(legs) ?? "Custom"),
    [mode, strategyId, legs],
  );
  ...
  /* ── Place the strategy as a paper trade (Paper tab only) ── */
  async function placeTrade() {
    if (!trade || !chain || legs.length === 0 || placing) return;
    setPlacing(true);
    setPlaceMsg(null);
    try {
      const res = await trade.onPlaceTrade(legs, { underlying: ticker, strategyName, netCost: analytics?.summary.netCost ?? 0 });
      setPlaceMsg(res);
    } catch (e) {
      setPlaceMsg({ ok: false, msg: e instanceof Error ? e.message : "Trade failed." });
    } finally {
      setPlacing(false);
    }
  }
                ...
                <>
                  <div className="rounded-md border border-border bg-card p-4">
                    <div className="flex items-center justify-between mb-2 gap-2">
                      <h3 className="text-sm font-medium text-foreground truncate">{strategyName}</h3>
                  ...
                  {trade && (
                    <TradePanel
                      ...
                      legCount={legs.filter((l) => l.type !== "stock").length}
                      placing={placing}
                      msg={placeMsg}
                      onPlace={placeTrade}
